...
# This server uses a global Python dictionary (system_state) to
# hold the simulation graph between requests. This is intentional
# for LOCAL, SINGLE-USER execution only (portfolio demo).
#
# If you deploy this with a multi-worker server (Gunicorn, etc.),
# each worker gets its own copy of this state and requests may
# hit different workers, causing the simulation to glitch.
#
# Production fix: Move state to Redis, SQLite, or return full
# state in every response (stateless API). For the demo, a
# single-worker Flask dev server is the correct use case.
# ================================================================
...
try:
    ...
    ML_ENABLED = False
    print(f"⚠️  AI Engine Missing ({e}). Running in Logic-Only mode.")
...
def generate_ccp_ledger(G):
    ...
    for u, v, attr in G.edges(data=True):
        if random.random() < 0.20:
            ...
            if sender.get('status') == 'Defaulted':
                status = "FAILED"
...
def check_circuit_breaker(G):
    ...
    if defaulted / total >= CIRCUIT_TRIGGER_DEFAULTS:
        return "HALTED"
    return "OPEN"
...
def calculate_stability_metrics(G):
    ...
    for n in G.nodes():
        if n in system_state['history'] and system_state['history'][n]:
            ...
            if len(hist) > 1:
                risk_slopes.append(hist[-1].get('slope', 0))

    if risk_slopes:
        avg_slope = np.mean(risk_slopes)
        nash_acceleration = max(0, avg_slope)

    acceleration_factor = nash_acceleration * 0.5
    butterfly += acceleration_factor
    butterfly = min(100, butterfly)
...
def graph_to_json(G):
    ...
    for n, attr in G.nodes(data=True):
        ...
        if vip and risk_label == "SAFE":
            color = "#3b82f6"

        node_status_map[n] = risk_label

        if n not in system_state['history']:
            system_state['history'][n] = []
        ...
        if len(hist_list) == 0 or hist_list[-1]['round'] != system_state['round']:
            ...
            if len(hist_list) > 50:
                system_state['history'][n] = hist_list[-50:]
    ...
    nodes.append({
        "id": "CCP_CORE", "name": "CCP_PRIME", "actual_assets": 999,
        "health": 100, "val": 1, "color": "#d946ef",
        "ml_risk_prob": 0, "risk_label": "SAFE", "is_ccp": True
    })
    ...
    for n in G.nodes():
        links.append({
            "source": n, "target": "CCP_CORE",
            "amount": 0, "color": "#d946ef", "stress": "Stable", "type": "membership"
        })
...
def health_check():
    """Health check endpoint. Frontend pings this before showing the Launch button."""
    return jsonify({
        "status": "online",
        "ml_enabled": ML_ENABLED,
        "round": system_state['round'],
        "initialized": system_state['graph'] is not None
    })
...
def track_node(node_id):
    if node_id not in system_state['history']:
        return jsonify({"error": "No history", "history": []})
...
def init_system():
    try:
        ...
        system_state['logs'] = ["SYSTEM RESET", f"CORE-ML: {'ACTIVE' if ML_ENABLED else 'OFFLINE (Logic Mode)'}"]
    ...
    try:
        ...
        for n in list(G.nodes()):
            ...
            if node['capital_ratio'] <= INSOLVENCY_LIMIT:
                ...
                defaults += 1
                new_logs.append(f"💀 DEFAULT: {n} insolvent.")
            else:
                active += 1
        ...
        if circuit == "HALTED":
            new_logs.append("🛑 MARKET HALTED. Circuit Breaker Triggered.")
